...
App = function () {
  function App() {_classCallCheck(this, App);
    ...
    {var
      ...
      tools.drawPath(ctx, function () {
        if (cfg.shadowBlur) {
          ctx.shadowColor = 'rgba(0,0,0,0.025)';
      ...
      i) {
        ...
        tools.drawPath(ctx, function () {
          ...
          ctx.fillStyle = 'rgba(0,0,0,' + per * 0.3 + ')';
    ...
    } }, { key: 'startGeneratingNumbers', value: function startGeneratingNumbers()
    {
      ...
      function generateItem() {var
        ...
        var item = {
          p: 0,
          color: 'rgba(0,0,0,' + tools.random(0.01, 0.3) + ')',
    ...
    } }, { key: 'drawAnimLines', value: function drawAnimLines()
    {
      ...
      state.animLines.forEach(function (line, i) {
        ...
        var p1 = p / 0.5;
        var p2 = 1 - (p - 0.5) / 0.5;
        var color = 'rgba(0,0,0,' + (0.1 + line.color * (p <= 0.5 ? p1 : p2)) + ')';
    ...
    } }, { key: 'startGeneratingGlitches', value: function startGeneratingGlitches()
    {
      ...
      function generateItem() {var
        ...
        var item = {
          p: 0,
          color: 'rgba(0,0,0,' + tools.random(0.01, 1) + ')',
    ...
    props) {
      ...
      if (vector <= mouseRange) {
        ...
        var color = 'rgba(0,0,0,' + 0.3 * percent + ')';
        var color2 = 'rgba(0,0,0,' + 0.7 * p2 * percent + ')';
        ...
        tools.drawPath(ctx, function () {
          ctx.strokeStyle = 'rgba(0,0,0,' + 0.5 * percent + ')';
        ...
        tools.drawPath(ctx, function () {
          ...
          ctx.strokeStyle = 'rgba(0,0,0,' + 0.75 * percent + ')';
    ...
    props) {
      ...
      if (!mf) return;
      tools.drawPath(ctx, function () {
        ctx.fillStyle = 'rgba(0,0,0,' + mf * 0.15 + ')';
    ...
    props) {var
      ...
      plane.centerCoords;
      tools.drawPath(ctx, function () {
        ctx.fillStyle = 'rgba(0,0,0,' + (0.2 + (1 - p) * 1) + ')';
    ...
    props) {var
      ...
      if (i) {
        tools.drawPath(ctx, function () {
          ctx.fillStyle = 'rgba(0,0,0,' + (0.05 + (1 - p) * 0.35) + ')';
    ...
    props) {
      ...
      var text = i - yCenter + '';
      ctx.fillStyle = 'rgba(0,0,0,' + (0.1 + (1 - f) * 0.75) + ')';
    ...
    props) {
      ...
      var textCoords = [x - ctx.measureText(text).width / 2, y + cfg.sectionWidth / 2 + cfg.numberOffset];
      tools.drawPath(ctx, function () {
        ctx.fillStyle = 'rgba(0,0,0,' + f * 0.5 + ')';
      ...
      tools.drawPath(ctx, function () {
        ...
        ctx.fillStyle = 'rgba(0,0,0,' + f * 0.35 + ')';
        ctx.textBaseline = 'top';
        ctx.fillText(
        text,
        textCoords[0],
    ...
    props) {
      ...
      tools.drawPath(ctx, function () {
        ...
        ctx.fillStyle = 'rgba(0,0,0,' + f * 0.3 + ')';
        ctx.textBaseline = 'middle';
        var textCoords = [x + cfg.sectionWidth / 2 + cfg.numberOffset, y + cfg.sectionHeight / 2];
        ctx.fillText(
        xCenter - i2 + '',
